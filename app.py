...
def _render_filters(min_date, max_date, lat_min, lat_max, lon_min, lon_max):
    ...
    with st.form("filters_form"):
        ...
        if _filter_type == "Прямоугольник":
            ...
            if not st.session_state.get("draw_mode_rect", False):
                ...
                if _drawn:
                    st.markdown(
                        f"<div style='font-size:0.82rem;color:#888;line-height:1.8;padding:4px 0;'>"
                    ...
                    st.caption("Нарисуйте прямоугольник на карте.")
    ...
    if _reset_filter:
        ...
        st.session_state.pop("draw_mode_rect", None)
        st.rerun(scope="fragment")
    ...
    if submitted:
        ...
        _new_circle = None
        if _filter_type == "Прямоугольник":
            if st.session_state.get("draw_mode_rect", False):
...
_draw_mode_active = (
    st.session_state.get("filter_type_select") == "Прямоугольник"
    and st.session_state.get("draw_mode_rect", False)
)
...
with tab_map:
    ...
    _drawn_bbox = render_map(df, bbox=bbox, circle=circle, df_stations=df_stations, draw_mode=_draw_mode_active)
    map_loader.empty()
    if _drawn_bbox is not None and _drawn_bbox != st.session_state.get("drawn_bbox"):
        st.session_state["drawn_bbox"] = _drawn_bbox
        st.session_state["bbox"] = _drawn_bbox
        st.rerun()
